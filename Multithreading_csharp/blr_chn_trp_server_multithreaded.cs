...
public static class ServerCommunication
{
	...
	public static void ReadDataFromDelhiServer()
	{
		for(Int32 counter=1; counter<=3600; counter++)
		{
			Console.WriteLine("DELHI SERVER: The server is reading the {0} data segment.",counter);
...
public class BengaluruServer
{
	...
	{
		Thread t1 = new Thread(new ThreadStart(ServerCommunication.ReadDataFromChennaiServer));
		Thread t2 = new Thread(new ThreadStart(ServerCommunication.WriteDataToTirupatiServer));
